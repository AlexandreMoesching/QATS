...
run <- function(name, xx_hat, time_s) {
  data.frame(
    ...
    l1        = lp_norm(xx.0, xx_hat, 1),
    l2        = lp_norm(xx.0, xx_hat, 2),
...
``` r
# Reshape to long format for faceted display
acc_long <- data.frame(
  Method = rep(results$Method, 4),
  Metric = rep(c("V-Measure", "l0 (error rate)", "l1", "l2"),
               each = nrow(results)),
  Value  = c(results$V_measure, results$l0, results$l1, results$l2)
)
acc_long$Metric <- factor(acc_long$Metric,
                          levels = c("V-Measure", "l0 (error rate)", "l1", "l2"))

ggplot(acc_long, aes(Method, Value)) +
